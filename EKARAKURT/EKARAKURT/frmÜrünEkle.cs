    ...
    public partial class frmÜrünEkle : Form
    {
        ...
        private void BarkodNotxt_TextChanged(object sender, EventArgs e)
        {
            if (string.IsNullOrWhiteSpace(BarkodNotxt.Text))
            {
                ...
                return; // Barkod boşsa, fonksiyondan çık
            }

            try
            {
                baglanti.Open();
                using (SqlCommand komut = new SqlCommand("SELECT * FROM urun WHERE barkodno = @barkod", baglanti))
                {
                    komut.Parameters.AddWithValue("@barkod", BarkodNotxt.Text.Trim());
